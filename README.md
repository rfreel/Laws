...
CI rejects `@unsafe`, `?TODO`, and unsafe-def shorthand, then checks `PROOF.bend` under Bend 2.0.27 installed from `pins/bend.json` via `tools/install_bend.sh`. Prefer `bash tools/install_bend.sh` locally.
